...
{
    return (uint32)IfxAsclin_Asc_getReadCount(&s_ascHandle);
}
...
uint32 UartXfer_Read(uint8 *pDest, uint32 maxLen)
{
    ...
    IfxAsclin_Asc_read(&s_ascHandle, pDest, &count, 0u);
    return (uint32)count;
}
...
uint32 UartXfer_ReadBlocking(uint8 *pDest, uint32 len, uint32 timeoutMs)
{
    ...
    while (got < len)
    {
        ...
        IfxAsclin_Asc_read(&s_ascHandle, &pDest[got], &remain, 0u);
        got += (uint32)remain;
...
void UartXfer_Write(const uint8 *pSrc, uint32 len)
{
    ...
    IfxAsclin_Asc_write(&s_ascHandle, pSrc, &count, TIME_INFINITE);
}
...
void UartXfer_FlushRx(void)
{
    uint8 dummy;
    Ifx_SizeT one = 1u;
    while ((uint32)IfxAsclin_Asc_getReadCount(&s_ascHandle) > 0u)
        IfxAsclin_Asc_read(&s_ascHandle, &dummy, &one, 0u);
}